...
def ffz_torsion_deform(vector, a=3, b=5, beta=0.1):
    ...
    # Convert to numpy before mean to avoid attribute error
    deformed_np = np.array(deformed).flatten().astype(float)
...
if __name__ == "__main__":
    # Generate data
    VOCAB_SIZE = 30  # Choose something larger than unique words to avoid truncation surprises
    ...
    plt.ylabel("BCE Loss")
    plt.show()
    # NLP Inference Example – Reuse training vocabulary!
    test_texts = ["This is amazing and great!", "This is terrible and bad."]

    # Vectorize test using THE SAME vocab order as training
    ...
    X_test = vectorize_with_vocab(test_texts, vocab)  # ← key change: pass training vocab

    print(f"X_test shape: {X_test.shape}")  # Should now be (2, 20)
